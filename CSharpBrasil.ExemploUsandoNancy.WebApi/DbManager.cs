using CSharpBrasil.ExemploUsandoNancy.WebApi.Models;
using LiteDB;
using System;
...
namespace CSharpBrasil.ExemploUsandoNancy.WebApi
{
    public static class DbManager
    {
        ...
        static DbManager()
        {
            ...
            var databasePath = Path.Combine(databaseDir, "ExemploUsandoNancy.db");
        ...
        public static List<Pagamento> GetPaymentByIdEstablishment(int id)
        {
            using (var db = new LiteDatabase(ConnectionString))
            {
                var model = db.GetCollection<Pagamento>().Find(x => x.IdEstabelecimento == id);
                return model.ToList();
            }
        }
